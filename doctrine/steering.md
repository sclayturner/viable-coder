...
What that geometry left implicit is the question this section answers. **What is the human doing while the middle runs?** The honest version of "you own the gate between the columns" — the line Section 3 carries through every chapter — does not survive contact with the actual pace of an agent crew unless something connects the two ends in flight. That something is steering.
...
- **Work-system reads the human** (Section 3 Ch 6 — the board's trailing signals) — the substrate that runs the work also records how the human held it. The board reads the human after the fact.
...
- Read the transcript as the work runs, not only at the gate. The transcript is the dashboard for the middle; the same way Section 1 ported instrumentation onto the self and Section 3 ports it onto the work-system reading the human.
...
- **Planted in Section 1 implicitly** — the two ends doctrine names what the human holds; this piece names how the human holds it through the middle.
- **Operationalized in Section 3 Ch 4** — the chapter that names the two non-transferable gates also names observability-of-the-middle and steering as what makes those gates holdable. The chapter's existing failure modes (origination by drift, review by rubber-stamp) are recast: both are what steering's absence looks like.
- **Configured in Section 3 Ch 5** — the produce-and-explain register is the primary steering instrument. Ch 5 is where the reader sets up the configuration that makes the middle observable for the work they are about to do.
- **Read on the long clock in Section 4** — declining reframe-vs-accept ratios, falling override rates on agent flags, and rising self-spec incidence are long-clock readings of steering depth across a career.
